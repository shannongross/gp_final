...
mod_summ_all_models2 <- mod_summ_all_models2 %>% mutate(
  ModName3=case_when(ModName=="DraftFinalModels2_2GIS_S"~"S (2GIS)",
                     ModName=="DraftFinalModels2_2GIS_UNC"~"UNC (2GIS)",
                     ModName=="DraftFinalModels2_2GIS_Unstrat"~"Unstrat (2GIS)",
                     ModName=="DraftFinalModels2_BaseModel_S"~"S (BaseModel)",
                     ModName=="DraftFinalModels2_BaseModel_UNC"~"UNC (BaseModel)",
                     ModName=="DraftFinalModels2_BaseModel_Unstrat"~"Unstrat (BaseModel)",
                     ModName=="DraftFinalModels2_NoGIS_S"~"S (NoGIS)",
                     ModName=="DraftFinalModels2_NoGIS_UNC"~"UNC (NoGIS)",
...
#   group_by(ModNameStratGroup, SiteCode, Dataset) %>%
#   summarise(n_visits=length(SiteCode),
#             mode=getmode(Prediction),
#             pct_mode_PvIvE=sum(Prediction==mode)/n_visits
#   ) %>%
#   ungroup() %>%
#   group_by(ModNameStratGroup, Dataset) %>%
#   summarise(AvgPctMode=mean(pct_mode_PvIvE)) %>% 
#   ungroup() 
# 
...
#                             levels=c("Testing","Training"))
# #reorder for clarity
# mod_summ2$ModNameStratGroup <- factor(mod_summ2$ModNameStratGroup,
#                              levels = c("Unstrat_noGIS","Unstrat",
#                                         "Strat_noGIS","Strat"))
# label_names <- c(
#   `PvIvE` = "Accuracy\nPvIvE",
...
#   ModName3=case_when(ModNameStratGroup=="Unstrat_noGIS"~"Unstrat (no GIS)",
#                      ModNameStratGroup=="Unstrat_allGIS"~"Unstrat (all GIS)",
#                      ModNameStratGroup=="Unstrat_2GIS"~"Unstrat (2 GIS)",
#                      ModNameStratGroup=="Strat_noGIS"~"Strat (no GIS)",
#                      ModNameStratGroup=="Strat_allGIS"~"Strat (all GIS)",
#                      ModNameStratGroup=="Strat_2GIS"~"Strat (2 GIS)"))
...
#   filter(ModNameStratGroup %in% c("Strat","Strat_noGIS","Unstrat",
#                          "Unstrat_noGIS")) %>%
#   filter(Measure %in% c("PvIvE","EvALI","Precision")) %>%
#   replace_na(list(Region_detail = "Unstrat"))
# 
...
#                                   levels = c("Unstrat","NE","SE"))
# 
# mod_summ_plot5<-ggplot(data=mod_summ_simple5, aes(x=ModNameStratGroup, y=Accuracy))+
#   # geom_point(aes(size=Dataset, color=Region_detail))+
...
#   group_by(ModNameStratGroup, Region_detail, Dataset) %>%
#   summarise(n_tests=length(SiteCode),
#             n_tests_wet=sum(wet),
#             n_tests_dry=sum(!wet),
...
#   # filter(SiteCode %in% revisited_sites) %>%
#   filter(TotalVisits > 1) %>% ##TODO: 
#   group_by(ModNameStratGroup, SiteCode, Dataset,Region_detail) %>%
#   summarise(n_visits=length(SiteCode),
#             mode=getmode(Prediction),
#             pct_mode_PvIvE=sum(Prediction==mode)/n_visits
#   ) %>%
#   ungroup() %>%
#   group_by(ModNameStratGroup, Dataset,Region_detail) %>%
#   summarise(AvgPctMode=mean(pct_mode_PvIvE)) %>% 
#   ungroup() 
...
#     ungroup() %>% 
#     filter(TimesSel>0), 
#   mapping = aes(x=Option, y=ModNameStratGroup)) +
#   geom_tile(aes(fill=factor(color_group)), color="grey30") +
#   scale_fill_manual(
#     values=c("grey"="grey",
#              # "white"="white",
...
#   ModNameStratGroup=="Unstrat_noGIS") %>% group_by(Dataset), 
...
#          ModNameStratGroup=="Unstrat_noGIS"))[1])
#        ) +
...
#   ModNameStratGroup=="Unstrat") %>% group_by(Dataset), 
#   aes(x=Region_detail, y=pct_correct_PvIvE*100, fill=Dataset)) + 
#   geom_bar(stat="identity", position=position_dodge(width = 1)) +
#   labs(title = "Unstratified Model (GIS)",
#        subtitle = "Classification % Correct between P vs I vs E",
#        caption=paste0("Total observations: ", dim(all_results_flat%>%filter(
#          ModNameStratGroup=="Unstrat"))[1])
#   ) +
#   geom_text( aes(label=round(pct_correct_PvIvE*100, digits=1)),
#              vjust=1.3, color="black", size=2.5, 
#              position=position_dodge(width = 1)) + 
...
#   ModNameStratGroup=="Strat") %>% group_by(Dataset), 
#   aes(x=Region_detail, y=pct_correct_PvIvE*100, fill=Dataset)) + 
#   geom_bar(stat="identity", position=position_dodge(width = 1)) +
#   labs(title = "Stratified Model (GIS)",
#        subtitle = "Classification % Correct between P vs I vs E",
#        caption=paste0("Total observations: ", dim(all_results_flat%>%filter(
#          ModNameStratGroup=="Strat"))[1])
#   ) +
#   geom_text( aes(label=round(pct_correct_PvIvE*100, digits=1)),
#              vjust=1.3, color="black", size=2.5, 
#              position=position_dodge(width = 1)) + 
...
#   ModNameStratGroup=="Strat_noGIS") %>% group_by(Dataset), 
#   aes(x=Region_detail, y=pct_correct_PvIvE*100, fill=Dataset)) + 
#   geom_bar(stat="identity", position=position_dodge(width = 1)) +
#   labs(title = "Stratified Model (No GIS)",
#        subtitle = "Classification % Correct between P vs I vs E",
#        caption=paste0("Total observations: ", dim(all_results_flat%>%filter(
#          ModNameStratGroup=="Strat_noGIS"))[1])
#   ) +
#   geom_text( aes(label=round(pct_correct_PvIvE*100, digits=1)),
#              vjust=1.3, color="black", size=2.5, 
#              position=position_dodge(width = 1)) + 
